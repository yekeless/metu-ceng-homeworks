...
namespace sym
{
	...
	{
		const Const *new_con = dynamic_cast<const Const *>(&other);
		if (new_con != nullptr)
		{
			return new_con->value_ == value_;
		}
		return false;
	}
}
...
namespace sym
{
	...
	{
		auto ptr = vars.find(var_);
		if (ptr != vars.end())
		{
			return new Const(ptr->second);
		}
		else
		{
			return new Var(var_);
	...
	{
		const Var *new_var = dynamic_cast<const Var *>(&other);
		if (new_var != nullptr)
		{
			return new_var->var_ == var_;
		}
		return false;
	}
}
