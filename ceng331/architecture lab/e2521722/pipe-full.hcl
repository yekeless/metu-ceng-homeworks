...
# I added JTAB instructions these locations. #
# word f_pc  since we need to jump the address at the end of jtab instruction, therefore we need to change pc according to this instruction like the ret instruction #
# instr_valid since it need to be a valid instruction #
# need_regids since it takes one register as an argument and it needs to access it #
# need_valC since it takes a constant value as a second argument #
# word d_srcB since it needs to read the value in the register and it should be the regB #
# word aluA since we need to chose constant value as an input to the aluA #
# word aluB since we need to chose the value which is in the register B as an input to the aluB #
# word mem_addr since we need to select the memory address #
# mem_read since we need to read the address of the jump point #
# F_stall since we need to wait untill the end of the jtab execution since we need to update pc according to its result  #
# D_bubble since we need to insert a bubble while we are completing the execution of jtab in order not to execute wrong instruction #
#/* $begin pipe-all-hcl */
####################################################################
...
word f_pc = [
	...
	W_icode == IRET || W_icode == IJTAB : W_valM;
